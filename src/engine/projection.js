...
// Returns a projector closure for the given camera orientation. It maps a
// world point [x, y, z] into camera space { x, y, z }, where z is the forward
// distance (used as the depth for blur and the split-image rangefinder).
export function makeProjector(yaw, pitch) {
  const cy = Math.cos(yaw), sy = Math.sin(yaw);
  const cp = Math.cos(pitch), sp = Math.sin(pitch);
  return (p) => {
    const rx = p[0], ry = p[1] - EYE_H, rz = p[2];
